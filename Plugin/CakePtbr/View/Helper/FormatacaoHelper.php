 ...
 */
	public function dataHora($dataHora = null, $segundos = true, $opcoes = array()) {
		...
		if ($segundos) {
			return $this->Time->format('d/m/Y �\s H:i:s', $dataHora, $config['invalid'], $config['userOffset']);
		}
		return $this->Time->format('d/m/Y �\s H:i', $dataHora, $config['invalid'], $config['userOffset']);
	}
 ...
 */
	public function dataCompleta($dataHora = null) {
		$_diasDaSemana = array('Domingo', 'Segunda-feira', 'Ter�a-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'S�bado');
 ...
 */
	public function diaSemana($dataHora = null) {
		$_diasDaSemana = array('DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'S�B');

		$dataHora = $this->_ajustaDataHora($dataHora);
		$w = date('w', $dataHora);

		return sprintf('%s', $_diasDaSemana[$w]);
	}
