 ...
 */
	public function dataHora($dataHora = null, $segundos = true, $opcoes = array()) {
		...
		if ($segundos) {
			return $this->Time->format('d/m/Y à\s H:i:s', $dataHora, $config['invalid'], $config['userOffset']);
		}
		return $this->Time->format('d/m/Y à\s H:i', $dataHora, $config['invalid'], $config['userOffset']);
	}
 ...
 */
	public function diaSemana($dataHora = null) {
		$_diasDaSemana = array('DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB');
