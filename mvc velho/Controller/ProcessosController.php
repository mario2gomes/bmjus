...
//            $tipoProcesso = $this -> Tipo_processo -> findById($id_tipo);
            $prazo = $processo['Tipo_processo']['prazo_processo'];
    ...
    function prorrogar() {
        ...
        if ($this-> request ->is('post')){
            ...
            if ($this-> Prorrogacao ->save($this->request->data)){
                ...
                $id = $this-> request -> data['Prorrogacao']['relatorio_id'];
                $qtd_dias = $this-> request -> data['Prorrogacao']['qtd_dias'];
                $this -> Relatorio -> id = $id;
                $prazo = $this -> Relatorio -> field('prazo');
                $novo_prazo = $qtd_dias + $prazo;
                $this -> Relatorio -> set(array('estado_id'=>3,'prazo'=>$novo_prazo));
                
                if ($this -> Relatorio -> save()){
                    $id2 = $this -> Relatorio -> field('processo_id');
                    $dataSourceProrrogacao -> commit();
                    $this-> Flash -> success('Processo prorrogado');
                    $this -> redirect(array('action' => 'detalhe',$id2));
                }else{
    ...
    function suspender() {
        ...
        if ($this-> request ->is('post')){
            ...
            if ($this-> Suspensao ->save($this->request->data)){
                ...
                $id = $this-> request -> data['Suspensao']['relatorio_id'];
                $this -> Relatorio -> id = $id;
                $this -> Relatorio -> set(array('estado_id'=>2));
